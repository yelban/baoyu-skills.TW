...
Roast 版基於普通版的話題骨架和素材，用毒舌、尖銳、挑釁的風格重寫。整體結構與普通版相同（統計區塊、開頭概覽、群友畫像、正文分類、@bot 答疑（毒舌值班版，如有）、結尾），但風格完全不同。痛點部分省略。僅當 `include_roast=true` 時生成。標題加 "毒舌版" 字尾。
...
- 正文話題標題可以改得更損
- 引用原話時配上辛辣點評
- @bot 答疑改為「毒舌值班版」（本批有 @bot 請求時才出現，見 SKILL.md Step 3.9，放結尾前；無則省略）：照樣把乾貨答出來，但裹上調侃、嘴硬、吐槽提問者的口吻，與 roast 整體一致；來源同樣只用群聊上下文 + 自有知識、不聯網，查不到就嘴硬地承認查不到；同守下方紅線。請求行措辭自由發揮，用調侃口吻點出提問者和請求即可，別套「又來了」這類固定句式。標題如 `🤖 bot 答疑（毒舌值班版）`，結構示意：
...
11. （本批有 @bot 請求時）兩版各有對應 @bot 答疑小節？普通版真誠有用、毒舌版帶刺仍有乾貨？無編造的即時資訊？
