...
# 指定服務商
/baoyu-imagine --prompt "一隻貓" --image cat.png --provider openai --model gpt-image-2

# Azure OpenAI（model 為部署名稱）
/baoyu-imagine --prompt "一隻貓" --image cat.png --provider azure --model gpt-image-2
...
/baoyu-imagine --prompt "一張影棚人像" --image portrait.png --provider replicate --model bytedance/seedream-4.5 --ar 3:2

# Replicate Wan 2.7 Image Pro
/baoyu-imagine --prompt "一張概念分鏡" --image frame.png --provider replicate --model wan-video/wan-2.7-image-pro --size 2048x1152
...
- Replicate 預設模型改為 `google/nano-banana-2`。`baoyu-imagine` 目前只對 `google/nano-banana*`、`bytedance/seedream-4.5`、`bytedance/seedream-5-lite`、`wan-video/wan-2.7-image` 和 `wan-video/wan-2.7-image-pro` 開啟本地能力識別與校驗。
