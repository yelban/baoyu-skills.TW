...
| `intuition-machine` | clean + cool + technical + dense | 技術檔案、學術 |
...
每個賬號擁有獨立的 Chrome 配置目錄，保證瀏覽器方式下的登入會話互不幹擾。API 憑證可在 EXTEND.md 中直接配置，也可透過 `.env` 檔案使用別名字首的環境變數（如 `WECHAT_BAOYU_APP_ID`）。
...
- MiniMax：`image-01` 支援官方檔案裡的自定義 `width` / `height`；`image-01-live` 更偏低延遲，適合配合 `--ar` 使用。
