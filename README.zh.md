...
寶玉分享的 AI Agent 技能集（適用於 Claude Code、Codex 等），提升日常工作效率。

## 作者的圖書

<img width="500" height="500" alt="圖解 Skill —— AI 提效實戰指南" src="https://github.com/user-attachments/assets/6caef6a2-6f11-490e-a43b-e810df8e9354" />

作者的圖書《圖解 Skill —— AI 提效實戰指南》系統講解如何設計、編寫、安裝和迭代 Skill，並配有完整示例、提示詞、插圖生成工作流和章節配套資源。

- 官方配套倉庫：[JimLiu/Illustrated-Agent-Skills](https://github.com/JimLiu/Illustrated-Agent-Skills)
- 購買連結：[京東購買](https://u.jd.com/RDY9YwC)
- 電子書購買連結：https://www.ituring.com.cn/book/3616
...
> **提示**：本倉庫已收錄 20+ 個 skill，請按需安裝你真正會用到的那幾個，不要一次性全裝 —— 每個載入的 skill 都會在 Agent 每次執行時佔用額外上下文。
...
如果只在某個專案中使用部分技能，不需要安裝整個外掛。Codex 會掃描專案裡的 `.agents/skills`，可以只把需要的 skill 按整個目錄複製或軟連結到當前專案：
...
不需要單獨安裝 `baoyu-markdown-to-html`，`baoyu-post-to-wechat` 已內建 Markdown 到公眾號可用 HTML 的轉換流程。只有需要先把原始文本或草稿整理成 Markdown 文章結構（標題、摘要、層級標題、加粗、列表等）時，再額外安裝 `baoyu-format-markdown`。

公眾號 API 憑證按作用範圍放置：

- 全域性：`~/.baoyu-skills/.env`
- 專案：`<project>/.baoyu-skills/.env`

專案級 `.env` 適合只給當前專案使用，注意不要提交到 Git。
...
> **TW fork 補充**：Plugin 更新是**兩階段**（marketplace 索引 + plugin 快取）。下面上游步驟只刷新 marketplace，要載入新內容還要更新 plugin 快取並重啟 Agent。完整指南含 slash 指令、手動暴力刷、版本驗證、scope（user vs local）故障排除：[docs/traditional-chinese-fork.md#更新已安裝的-plugin](docs/traditional-chinese-fork.md#更新已安裝的-plugin)。

更新技能到最新版本：

1. 在 Agent 中執行 `/plugin`
...
如果你想讓本地 Agent 直接做設計，可以試試 [JimLiu/baoyu-design](https://github.com/JimLiu/baoyu-design)。這是一個獨立專案，它把 Claude Design 打包成可移植的 Agent Skill，可在 Cursor、Claude Code、Codex、Claude Desktop 或其他能讀寫檔案的編碼 Agent 中執行，用來生成精緻 UI 稿、可互動原型、線框圖、落地頁、儀表盤、移動 App 和幻燈片，產物都是自包含 HTML，留在你自己的倉庫裡。

<a href="https://github.com/JimLiu/baoyu-design">
  <img src="https://raw.githubusercontent.com/JimLiu/baoyu-design/main/assets/screenshots/cursor-reader-mac-app.webp" alt="Cursor 執行 baoyu-design" width="720">
</a>
...
從源素材生成可直接釋出的 SVG 圖表 —— 包括流程圖、時序/協議圖、架構/結構圖、示意圖（直覺圖解）。分析輸入素材，推薦圖表型別和拆分策略，一次確認後批次生成。Claude 直接輸出符合統一設計規範的真實 SVG 程式碼，產物是自包含的 `.svg` 檔案，內嵌樣式並自動支援深色模式。

```bash
# 主題描述 —— 技能分析並提出方案
/baoyu-diagram "JWT 認證流程是怎麼工作的"
/baoyu-diagram "Kubernetes 架構" --type structural
...
| 引數 | 說明 |
|------|------|
| `--type <name>` | `flowchart`、`sequence`、`structural`、`illustrative`、`class`、`auto`（預設）。跳過型別推薦直接生成。 |
| `--lang <code>` | 輸出語言（en、zh、ja 等） |
| `--out <path>` | 輸出檔案路徑。生成聚焦於最重要內容的單張圖表。 |

**五種圖表型別**：

| 型別 | 適用場景 | 觸發動詞 |
|------|----------|----------|
| `flowchart` | 按順序走一遍流程 | 流程、步驟、工作流、生命週期、狀態機 |
| `sequence` | 誰和誰通訊、按什麼順序 | 協議、握手、認證流程、OAuth、TCP、請求/響應 |
| `structural` | 展示什麼包含什麼、如何組織 | 架構、元件、拓撲、佈局、什麼在什麼裡面 |
| `illustrative` | 建立直覺 —— 畫出機制本身 | 怎麼工作、原理、為什麼、直觀解釋 |
| `class` | 型別是什麼、它們如何關聯 | 類圖、UML、繼承、介面、資料模型 |

本技能不呼叫任何影像生成模型 —— Claude 通過手算座標直接寫 SVG 程式碼，確保每個圖表都遵守設計規範。內嵌的 `<style>` 塊包含 `@media (prefers-color-scheme: dark)`，同一個檔案在淺色和深色模式下均正確渲染，可嵌入到任意支援 SVG 的宿主環境中。
...
純文本輸入預設按普通帖子處理，Markdown 檔案預設按 X 文章處理。指令碼會將內容填入瀏覽器，使用者需手動檢查併發布。
...
| 方式 | 速度 | 要求 |
|------|------|------|
| API（推薦） | 快 | API 憑證（本機 IP 在公眾號白名單內） |
| 瀏覽器 | 慢 | Chrome，登入會話 |
| 遠端 API | 快 | API 憑證 + 一臺 IP 在公眾號白名單內、可 SSH 登入的伺服器 |

**API 配置**（更快的釋出方式）：

```bash
# 新增到 .baoyu-skills/.env（專案級）或 ~/.baoyu-skills/.env（使用者級）
...
**遠端 API 方式**（適用於本機 IP 不在公眾號白名單內的情況）：通過 SSH SOCKS5 動態埠轉發，將對 `api.weixin.qq.com` 的 HTTPS 呼叫轉發到 IP 在白名單內的伺服器上。Markdown 渲染、圖片處理、草稿組裝仍在本地完成；遠端不會落任何檔案，`AppSecret` 不會離開本地程序。僅支援 SSH 金鑰認證，且只接受型別化的 `remote_publish_*` 配置項，不透傳任意 ssh 選項。在 EXTEND.md 中配置 `remote_publish_host` 等欄位後，釋出時加上 `--remote`（或將 `default_publish_method` 設為 `remote-api`）。

**多賬號支援**：通過 `EXTEND.md` 管理多個微信公眾號：

```bash
mkdir -p .baoyu-skills/baoyu-post-to-wechat
```

建立 `.baoyu-skills/baoyu-post-to-wechat/EXTEND.md`：
...
每個賬號擁有獨立的 Chrome 配置目錄，保證瀏覽器方式下的登入會話互不干擾。API 憑證可在 EXTEND.md 中直接配置，也可通過 `.env` 檔案使用別名字首的環境變數（如 `WECHAT_BAOYU_APP_ID`）。
...
基於 AI SDK 的影像生成，支援 OpenAI GPT Image 2、Azure OpenAI、Google、OpenRouter、DashScope（阿里通義萬相）、MiniMax、即夢（Jimeng）、豆包（Seedream）和 Replicate API。支援文生圖、參考圖、寬高比、自定義尺寸、批次生成和質量預設。
...
# 指定服務商
/baoyu-image-gen --prompt "一隻貓" --image cat.png --provider openai --model gpt-image-2

# Azure OpenAI（model 為部署名稱）
/baoyu-image-gen --prompt "一隻貓" --image cat.png --provider azure --model gpt-image-2

# OpenRouter
/baoyu-image-gen --prompt "一隻貓" --image cat.png --provider openrouter

# OpenRouter + 參考圖
/baoyu-image-gen --prompt "把它變成藍色" --image out.png --provider openrouter --model google/gemini-3.1-flash-image --ref source.png

# DashScope（阿里通義萬相）
/baoyu-image-gen --prompt "一隻可愛的貓" --image cat.png --provider dashscope

# DashScope 自定義尺寸
/baoyu-image-gen --prompt "為咖啡品牌設計一張 21:9 橫幅海報，包含清晰中文標題" --image banner.png --provider dashscope --model qwen-image-2.0-pro --size 2048x872

# Z.AI GLM-Image
/baoyu-image-gen --prompt "一張帶清晰中文標題的科技海報" --image out.png --provider zai
...
# Replicate（預設：google/nano-banana-2）
/baoyu-image-gen --prompt "一隻貓" --image cat.png --provider replicate

# Replicate Seedream 4.5
/baoyu-image-gen --prompt "一張影棚人像" --image portrait.png --provider replicate --model bytedance/seedream-4.5 --ar 3:2

# Replicate Wan 2.7 Image Pro
/baoyu-image-gen --prompt "一張概念分鏡" --image frame.png --provider replicate --model wan-video/wan-2.7-image-pro --size 2048x1152

# 即夢（Jimeng）
/baoyu-image-gen --prompt "一隻可愛的貓" --image cat.png --provider jimeng

# 豆包（Seedream）
/baoyu-image-gen --prompt "一隻可愛的貓" --image cat.png --provider seedream
...
| `--size` | 尺寸（如 `1024x1024`；`gpt-image-2` 支援最長邊不超過 3840px 的有效自定義尺寸） |
| `--quality` | `normal` 或 `2k`（預設：`2k`） |
| `--imageSize` | Google/OpenRouter 使用的 `1K`、`2K`、`4K` |
| `--imageApiDialect` | OpenAI 相容閘道器的影像 API 方言（`openai-native` 或 `ratio-metadata`） |
| `--ref` | 參考圖片（Google、OpenAI、Azure OpenAI、OpenRouter、Replicate 支援的模型家族、MiniMax 或 Seedream 5.0/4.5/4.0） |
| `--n` | 單次請求生成圖片數量（`replicate` 當前只支援 `--n 1`） |
| `--json` | 輸出 JSON 結果 |
...
| `ZAI_API_KEY` | Z.AI API 金鑰 | - |
| `BIGMODEL_API_KEY` | Z.AI API 金鑰向後相容別名 | - |
...
| `AZURE_OPENAI_IMAGE_MODEL` | 相容舊配置的 Azure 部署/模型別名 | `gpt-image-2` |
...
| `BIGMODEL_IMAGE_MODEL` | Z.AI 模型向後相容別名 | `glm-image` |
...
| `OPENAI_IMAGE_API_DIALECT` | OpenAI 相容影像 API 方言（`openai-native` 或 `ratio-metadata`） | `openai-native` |
...
| `ZAI_BASE_URL` | 自定義 Z.AI 端點 | `https://api.z.ai/api/paas/v4` |
| `BIGMODEL_BASE_URL` | Z.AI 端點向後相容別名 | - |
| `MINIMAX_BASE_URL` | 自定義 MiniMax 端點 | `https://api.minimaxi.com` |
...
- Z.AI：`glm-image` 適合海報、圖表和中英文排版密集的圖片生成，暫不支援參考圖。
...
- Replicate 預設模型改為 `google/nano-banana-2`。`baoyu-image-gen` 目前只對 `google/nano-banana*`、`bytedance/seedream-4.5`、`bytedance/seedream-5-lite`、`wan-video/wan-2.7-image` 和 `wan-video/wan-2.7-image-pro` 開啟本地能力識別與校驗。
- Replicate 當前只儲存單張輸出圖，`--n > 1` 會在本地直接報錯，避免多圖結果被靜默丟棄。
- Replicate 的引數能力按模型家族區分：nano-banana 走 `--quality` / `--ar`，Seedream 走校驗後的 `--size` / `--ar`，Wan 走校驗後的 `--size`（`--ar` 會先在本地換算成具體尺寸）。

**服務商自動選擇**：
1. 如果指定了 `--provider` → 使用指定的
2. 如果傳了 `--ref` 且未指定 provider → 依次嘗試 Google、OpenAI、Azure、OpenRouter、Replicate、Seedream、MiniMax，最後是 Agnes
3. 如果只有一個 API 金鑰 → 使用對應服務商
4. 如果多個可用 → 預設使用 Google，然後依次為 OpenAI、Azure、OpenRouter、DashScope、Z.AI、MiniMax、Replicate、即夢、豆包、Agnes

#### baoyu-danger-gemini-web

與 Gemini Web 互動，生成文本和圖片。
...
下載 YouTube 影片字幕/轉錄文本和封面圖片。支援多語言、翻譯、章節分段和說話人識別。快取原始資料以便快速重新格式化。
...
通過 Chrome CDP 抓取任意 URL 並轉換為 Markdown。同時儲存渲染後的 HTML 快照，Defuddle 失敗時自動回退到舊版提取器。
...
格式化純文本或 Markdown 檔案，新增 frontmatter、標題、摘要、層級標題、加粗、列表和程式碼塊。
...
微信群聊精華提取。使用 [wx-cli](https://github.com/jackwener/wx-cli) 從群訊息中提取話題、引言和統計資料，生成結構化簡報。支援跨次執行的群聊歷史、群友畫像和群級事實記憶維護，可生成正常版和毒舌版，並在簡報中回應群裡向 `@bot` 提出的問題。

```bash
# 總結群最近訊息
/baoyu-wechat-summary 相親相愛一家人 最近 1 天

# 週報
/baoyu-wechat-summary AI 技術群 最近 7 天

# 增量模式（從上次摘要繼續）
/baoyu-wechat-summary 相親相愛一家人

# 毒舌版
/baoyu-wechat-summary 相親相愛一家人 最近 3 天 毒舌版
```

**前置要求**：
- 安裝 [wx-cli](https://github.com/jackwener/wx-cli)（`npm install -g @jackwener/wx-cli`）
- macOS 上執行並登入 WeChat 4.x

**特性**：
- 話題提取，帶歸屬和引言
- 發言排行榜和群友畫像
- 群級事實記憶：群友指正過的事實跨期生效（內建防注入規則）
- 增量模式（從上次摘要斷點繼續）
- 大批次訊息自動按天分割
- 正常版和毒舌版兩種風格
- 支援從歷史摘要回溯初始化畫像
...
從任意已安裝的 Electron 應用的 `app.asar` 中提取資源和 JavaScript。當 `.js.map` 內嵌 `sourcesContent` 時，還原原始原始碼樹（含 TypeScript/JSX）；否則用 Prettier 原地美化壓縮後的 JS/CSS。始終跳過 `node_modules`。支援 macOS 和 Windows，其他平臺請用 `--asar <path>` 指定 asar 檔案。

```bash
# 按應用名提取（預設輸出：~/Downloads/<AppName>-electron-extract/）
/baoyu-electron-extract Codex

# 按絕對路徑提取（.app 包、安裝目錄或 .asar 檔案均可）
...
| 選項 | 說明 | 預設值 |
|------|------|--------|
| `<app>` | 應用名或絕對路徑（未給 `--asar` 時必填） | — |
| `--output`, `-o` | 輸出目錄 | `~/Downloads/<AppName>-electron-extract` |
| `--asar` | 覆蓋解析得到的 `.asar` 路徑 | 自動發現 |
| `--force`, `-f` | 允許寫入非空的已有輸出目錄 | false |
| `--skip-format` | 跳過 Prettier 格式化 | false |
| `--skip-restore` | 跳過 source-map 還原 | false |
| `--no-unpacked` | 不復制同級的 `app.asar.unpacked/` | false |
| `--dry-run` | 列印解析路徑後退出，不寫檔案 | false |
| `--json` | 在 stdout 輸出一行 JSON 概要 | false |

**輸出結構**：`extract-report.json`（計數、警告、路徑），`extracted/`（asar 原始內容，無 map 時原地美化），`extracted.unpacked/`（存在時複製的原生模組），以及 `restored/`（基於 `.js.map` 重建的原始碼樹）。
...
所有技能支援通過 `EXTEND.md` 檔案自定義。建立擴充套件檔案可覆蓋預設樣式、新增自定義配置或定義個人預設。
...
**警告：** 本專案通過瀏覽器 cookies 使用非官方 API。使用風險自負。
...
**代理配置**：如果需要通過代理訪問 Google 服務（如中國大陸使用者），請在命令前設定環境變數：
...
- [architecture-diagram-generator](https://github.com/Cocoon-AI/architecture-diagram-generator) by [@Cocoon-AI](https://github.com/Cocoon-AI) — 圖表技能設計體系的靈感來源

## 許可證

除另有說明外，本倉庫採用 [MIT License](./LICENSE) 授權。

釋出到 ClawHub 的 skill 根據 ClawHub registry 規則以 `MIT-0` 分發。第三方程式碼和素材按其註明的原始許可授權。

## Star History

[![Star History Chart](https://api.star-history.com/svg?repos=yelban/baoyu-skills.TW&type=Date)](https://www.star-history.com/#yelban/baoyu-skills.TW&Date)
