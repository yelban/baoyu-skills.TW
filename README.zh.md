...
4. 如果多個可用 → 預設使用 Google，然後依次為 OpenAI、Azure、OpenRouter、DashScope、Z.AI、MiniMax、Replicate、即夢、豆包
...
微信群聊精華提取。使用 [wx-cli](https://github.com/jackwener/wx-cli) 從群訊息中提取話題、引言和統計資料，生成結構化簡報。支援跨次執行的群聊歷史和群友畫像維護，可生成正常版和毒舌版，並在簡報中回應群裡向 `@bot` 提出的問題。
