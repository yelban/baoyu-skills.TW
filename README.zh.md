...
本技能不呼叫任何影像生成模型 —— Claude 透過手算座標直接寫 SVG 程式碼，確保每個圖表都遵守設計規範。內嵌的 `<style>` 塊包含 `@media (prefers-color-scheme: dark)`，同一個檔案在淺色和深色模式下均正確渲染，可嵌入到任意支援 SVG 的宿主環境中。
...
# OpenRouter + 參考圖
/baoyu-image-gen --prompt "把它變成藍色" --image out.png --provider openrouter --model google/gemini-3.1-flash-image-preview --ref source.png
...
微信群聊精華提取。使用 [wx-cli](https://github.com/jackwener/wx-cli) 從群訊息中提取話題、引言和統計資料，生成結構化簡報。支援跨次執行的群聊歷史和群友畫像維護，可生成正常版和毒舌版。
