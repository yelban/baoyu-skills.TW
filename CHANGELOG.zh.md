...
### 修復
- `codex-imagegen`：阻止 Codex 通過複製 `generated_images` 中無關舊圖片來代替真正生成的取巧行為。校驗改為要求當前執行緒內 `image_gen` 真正執行的證據——事件流中的 `image_gen` 事件，**或**當前執行緒 `generated_images` 目錄下新生成的 PNG——並在派生 agent 指令中禁止在呼叫 `image_gen` 之前讀取或複用歷史圖片（[#185](https://github.com/JimLiu/baoyu-skills/issues/185)）

### 文件
- README：補充 Codex 專案級安裝（`.agents/skills`）以及專案級與全域性公眾號憑證的放置位置

## 2.5.1 - 2026-06-13

### 文件
- 圖片生成類 skills：在共享後端選擇規則中補充 Cursor 原生 `GenerateImage` 後端說明，包括通過提示詞宣告寬高比、處理工具託管輸出檔案，以及使用 `reference_image_paths` 傳入參考圖。

## 2.5.0 - 2026-06-12

### 新功能
- `baoyu-wechat-summary`：新增群級事實記憶（`memory.md`）。群友指正過、確認過的客觀事實（如某個報錯提示的真實原因、某產品名的正確寫法）跨期生效——上一期摘要說錯、被群友指正的說法，後續不再重犯。寫入保守：三條門檻全滿足才記（針對具體事實 + 有理由或證據 + 無人反駁），嚴格防注入（只記陳述句事實、不記對 bot 的行為指令，`@bot` 不是白名單通道），每條帶出處，並有修訂/作廢/去重規則和 30 條上限。事實記憶由正常版和毒舌版共用
- `baoyu-image-gen`：新增 Agnes AI 圖片生成 provider (by @Davidlaizz)
- `baoyu-image-gen`：預設使用 GA Gemini 圖片端點，並支援搭配參考圖使用 (by @hypn4)
- `baoyu-post-to-wechat`：支援 `content_source_url`，用於設定"閱讀原文"連結 (by @NTLx)

### 修復
- `baoyu-image-gen`：移除 `resolveSize` 中的 gcd 計算，修復小數寬高比被扭曲的問題 (by @Davidlaizz)
- `baoyu-fetch`：`parseYouTubeVideoId` 支援解析 YouTube `/embed/` 連結 (by @Osamaali313)
- `baoyu-image-gen`：容忍 Windows 上 Bun 的 `mkdir(recursive)` 丟擲 `EEXIST` 錯誤 (by @sandypoli-boop)

### 文件
- 最佳化各 skill 的 description，提高觸發準確度 (by @yanghaod2278827)
- README：新增圖書/電子書連結，並在技能列表中突出 `baoyu-design`

## 2.4.1 - 2026-06-01

### 修復
- `baoyu-md` 與 `baoyu-chrome-cdp`：將 npm 包版本提升到 `0.1.1`，並重新整理依賴它們的 skill 依賴約束與 lockfile，確保全新安裝會拿到 Mermaid 子匯出、打包的 `assets/mermaid.min.js`，以及 Markdown Mermaid 預處理匯出。影響範圍包括 `baoyu-markdown-to-html`、`baoyu-post-to-wechat`、`baoyu-post-to-weibo`、`baoyu-post-to-x`、`baoyu-danger-gemini-web`、`baoyu-danger-x-to-markdown`（[#172](https://github.com/JimLiu/baoyu-skills/issues/172)）

## 2.4.0 - 2026-05-29

### 新功能
- `baoyu-wechat-summary`：為正常版和毒舌版簡報新增「@bot 答疑」小節。識別群裡提及 `@bot` / `@精華bot`（可通過新增的 `bot_aliases` 偏好自定義）的提問/請求，在專門小節中逐條回應——正常版真誠有用，毒舌版帶刺但仍給乾貨。答覆只依據群聊上下文和模型自有知識（不聯網），需要即時資料的會如實註明

## 2.3.0 - 2026-05-28

### 新功能
- `baoyu-md`、`baoyu-markdown-to-html`、`baoyu-post-to-wechat`、`baoyu-post-to-x`：支援 Obsidian 圖片 wikilink（`![[...]]`），可與標準 Markdown 圖片混用並保持圖片順序，同時解析 Obsidian 預設的 `Attachments/` 路徑 (by @zcqqq)

### 修復
- `baoyu-md`、`baoyu-post-to-x`：解析本地圖片路徑前先解碼 URL 編碼，正確處理包含空格或中文等字元的檔名 (by @zcqqq)
- `baoyu-md`、`baoyu-post-to-x`：加固圖片路徑解碼邏輯，遇到異常百分號轉義時安全回退，避免中斷佔位符提取 (by @zcqqq)

## 2.2.1 - 2026-05-26

### 文件
- `baoyu-image-gen`：在 SKILL.md 的 Usage 段落中顯式展示 `scripts/build-batch.ts`，使 `outline.md` + `prompts/` → `batch.json` 流程（例如 `baoyu-article-illustrator` 的產物）能與 `--batchfile` 並列被發現。明確 SKILL.md 中所有 `scripts/...` 路徑均相對 `{baseDir}`，並把 Generation Mode 表中的指引改為 `{baseDir}/scripts/build-batch.ts`。`build-batch.ts --help` 同步列印 `bun` / `npx -y bun` 呼叫，與 skill 其它指令碼統一使用 `{baseDir}/scripts/...` 寫法

## 2.2.0 - 2026-05-25

### 新功能
- `baoyu-image-gen`：新增 `codex-cli` provider，封裝 `codex-imagegen` 後端，讓 Codex 內建的 `image_gen` 工具也能走標準的 `--provider` / 批次 / EXTEND.md 流程。使用使用者自己的 Codex 訂閱,無需 `OPENAI_API_KEY`。新增環境變數 `BAOYU_CODEX_IMAGEGEN_{BIN,CACHE_DIR,TIMEOUT_MS,RETRIES,LOG_FILE}`；`BAOYU_IMAGE_GEN_<PROVIDER>_*` 類覆蓋項支援帶連字元的 provider 名稱(如 `BAOYU_IMAGE_GEN_CODEX_CLI_CONCURRENCY`)。`codex-cli` **不會**自動選用,需通過 `--provider codex-cli` 或 EXTEND.md 的 `default_provider: codex-cli` 顯式指定

### 重構
- `codex-imagegen`：從 `scripts/codex-imagegen/` + `scripts/codex-imagegen.sh` 拆分為獨立的 workspace 包 `packages/baoyu-codex-imagegen/`。bash 包裝器已移除,`src/main.ts` 自帶 `#!/usr/bin/env bun` shebang,作為唯一入口(`bun packages/baoyu-codex-imagegen/src/main.ts …`,若 `PATH` 中無 bun 則 `npx -y bun …`)。新增 `scripts/sync-codex-imagegen.sh` 同步 `skills/baoyu-image-gen/scripts/codex-imagegen/`,保持 skill 自包含

### 文件
- `baoyu-cover-image`：`SKILL.md` 中較長的 `scripts/codex-imagegen.sh` 內聯呼叫塊替換為指向 `references/codex-imagegen.md` 的連結,文件明確優先走 `baoyu-image-gen --provider codex-cli`、並保留直接呼叫 wrapper 的回退路徑
- `baoyu-article-illustrator`、`baoyu-comic`、`baoyu-infographic`、`baoyu-slide-deck`、`baoyu-xhs-images`：在各自 backend 選擇階梯中新增 `Codex via codex exec` 分支,並附帶 per-skill 的 `references/codex-imagegen.md`(優先路徑、回退、stdout schema、批次語義)
- `docs/codex-imagegen-backend.md` 與 `CLAUDE.md`：同步新的 workspace 路徑與呼叫示例

## 2.1.0 - 2026-05-24

### 新功能
- `baoyu-markdown-to-html`：在標準圖片佔位符流水線之前,通過共享 Chrome（CDP）把 ` ```mermaid ` 圍欄程式碼塊渲染為本地 PNG。新增 CLI 引數 `--mermaid-theme <default|forest|dark|neutral|base>`、`--mermaid-scale <N>`（預設 `2`,即 @2x 解析度）、`--mermaid-bg <white|transparent|#hex>`,以及用於關閉渲染的 `--no-mermaid`。生成的圖片落在 `imgs/.mermaid-cache/mermaid-<hash>.png`,跨次執行通過對 `(code, theme, scale, background, mermaid 版本)` 取 SHA-256 前 12 位進行去重/複用。Chrome 不可用或單塊渲染失敗時,保留瀏覽器側 `<pre class="mermaid">` 兜底
- `baoyu-post-to-wechat`、`baoyu-post-to-weibo`、`baoyu-post-to-x`:在微信 / 微博 / X 釋出流水線中同步接入上述 Mermaid → PNG 預處理,Mermaid 圖能以真圖形式出現在已釋出的稿件中(此前會落到未渲染的 `<pre>` 塊)。現有的 `WECHATIMGPH_*` / `WBIMGPH_*` / `XIMGPH_*` 佔位符流水線無需改動,會直接拾取生成的 PNG
- `baoyu-md` 包:新增匯出 `preprocessMermaidInMarkdown`、`extractMermaidBlocks`、`replaceMermaidBlocks`、`hashMermaidCode`、`MERMAID_VERSION`(渲染函式由 skill 注入,本包不再反向依賴 Chrome)
- `baoyu-chrome-cdp` 包:新增 `./mermaid` 子匯出,提供 `renderMermaidToPng(code, outputPath, options)`,底層為程序級單例 CDP 連線,複用共享 Chrome profile,並把 Mermaid 10.9.1 UMD 資產打入包內

## 2.0.1 - 2026-05-24

### 修復
- `baoyu-post-to-wechat`：釋出 `newspic`（圖文）時，將 `content` 欄位從 HTML 轉換為純文本後再提交微信草稿介面。此前直接傳 HTML，多圖場景下內容長度超出限制會觸發錯誤 45166（invalid content）。新增 `htmlToPlainText` 工具：把 `<br>` 和塊級閉合標籤轉成換行、剝離其餘標籤、解碼命名實體與十進位制/十六進位制數字實體、合併多餘空白 (by @Go1dFinger, [#164](https://github.com/JimLiu/baoyu-skills/pull/164), 關聯 [#163](https://github.com/JimLiu/baoyu-skills/issues/163))

### 致謝
- `baoyu-post-to-wechat` 的 newspic 純文本修復由 @Go1dFinger 貢獻（[#164](https://github.com/JimLiu/baoyu-skills/pull/164)）

## 2.0.0 - 2026-05-24

### 破壞性變更
- 移除 `baoyu-imagine` skill。所有功能（providers、指令碼、references）合併入 `baoyu-image-gen`，`marketplace.json` 改用新名稱註冊，`homepage` 連結更新為 `#baoyu-image-gen`
- 移除 `baoyu-image-cards` skill。所有功能（樣式、佈局、配色、預設）合併入 `baoyu-xhs-images`，`marketplace.json` 改用新名稱註冊
- 其它 skill（`baoyu-article-illustrator`、`baoyu-comic`、`baoyu-cover-image`、`baoyu-infographic`、`baoyu-slide-deck`）中 `## Image Generation Tools` 示例統一改用 `baoyu-image-gen`，不再引用 `baoyu-imagine`

### 遷移說明
- 舊的 `~/.baoyu-skills/baoyu-imagine/EXTEND.md` 與 `.baoyu-skills/baoyu-imagine/EXTEND.md` 配置檔案會被 `scripts/main.ts` 中的 legacy-path 解析器自動重新命名到 `…/baoyu-image-gen/EXTEND.md`，首次執行時遷移
- 通過斜槓命令呼叫時，請將 `/baoyu-imagine ...` 改為 `/baoyu-image-gen ...`；將所有 `baoyu-image-cards` 改為 `baoyu-xhs-images`

## 1.119.0 - 2026-05-24

### 新功能
- `baoyu-electron-extract`：新增 skill，可從任意已安裝的 Electron 應用的 `app.asar` 中提取資源和 JavaScript。`.js.map` 檔案內嵌 `sourcesContent` 時還原原始原始碼（含 TypeScript/JSX），否則用 Prettier 原地美化壓縮後的 JS/CSS。source-map 路徑先相對各 `.js.map` 檔案解析，因此 `../../src/main.ts` 這類打包器相對路徑會還原為 `restored/` 下的可讀路徑，而不是雜湊佔位符。始終跳過 `node_modules` 和 `webpack/runtime/*` 條目。macOS 下自動從 `/Applications` 和 `~/Applications` 發現應用，Windows 下從 `%LOCALAPPDATA%\Programs`、`%PROGRAMFILES%`、`%PROGRAMFILES(X86)%`、`%APPDATA%` 發現；其他平臺請用 `--asar <path>` 顯式指定。安全：拒絕寫入 `/`、使用者主目錄或當前工作目錄，未加 `--force` 時拒絕寫入非空的已有輸出目錄

## 1.118.0 - 2026-05-21

### 新功能
- `codex-imagegen`：新增面向非 Codex 執行時（如 Claude Code）的影像生成後端 —— 通過 `codex exec --json --sandbox danger-full-access` 呼叫 Codex CLI 內建的 `image_gen` 工具，無需 `OPENAI_API_KEY`。內建冪等快取、檔案鎖併發控制、JSONL 事件流解析、PNG 魔術位元組校驗和指數退避重試 (by @yelban, #158)
- `baoyu-cover-image`：在 `SKILL.md` 中接入 `codex-imagegen` 包裝指令碼（當 `preferred_image_backend: codex-imagegen` 時生效），並補充慢網路下的 `--timeout` 引數說明

### 重構
- `codex-imagegen`：在程式碼中強制校驗 `--prompt` 與 `--prompt-file` 互斥（此前僅在文件說明）
- `codex-imagegen`：將 `(opts as any).__promptFile` 這一 hack 改為 `CliOptions` 上型別化的 `promptFile` 欄位
- `codex-imagegen`：用複用的 `findCpToTarget` 輔助函式替換內聯的 `cp|mv ... generated_images` 正則
- `codex-imagegen`：錯誤返回時正確透傳 `attempts`（此前硬編碼為 `0`）
- `codex-imagegen`：刪除無用的 `parseFinalJson()` 函式及對應測試（包裝指令碼以磁碟校驗為準，不再依賴 agent 自報 JSON）

### 安全
- `codex-imagegen`：在拼入發往 `codex exec --sandbox danger-full-access` 的 agent 指令前，拒絕包含 shell 元字元的 `--image` / `--ref` 路徑

### 致謝
- `codex-imagegen` 後端由 @yelban 貢獻 (#158)

## 1.117.5 - 2026-05-21

### 致謝
- `baoyu-post-to-wechat`：遠端 API 釋出更新感謝 Dame5211 <1079825614@qq.com>

## 1.117.4 - 2026-05-21

### 新功能
- `baoyu-post-to-wechat`：新增通過 SSH SOCKS5 隧道進行遠端 API 釋出

### 修復
- `baoyu-post-to-wechat`：修復遠端 API 釋出在 Bun 下的執行問題，並嚴格校驗遠端釋出配置

### CI
- 測試前安裝 `baoyu-post-to-wechat` 指令碼依賴

## 1.117.3 - 2026-05-20

### 新功能
- CI：新增 skill 釋出提交校驗 —— 涉及 `skills/<name>/**` 的提交必須使用 Conventional Commit 格式；釋出/同步時校驗 SKILL.md 版本一致性

### 修復
- `baoyu-diagram`：為 SKILL.md 新增 version 欄位
- `baoyu-post-to-wechat`：同步 SKILL.md 版本

### 文件
- `baoyu-wechat-summary`：重構 profile 欄位 —— 將 `aliases` 拆分為 `group_nicknames`（使用者歷史群名）和 `aliases`（其他成員對使用者的稱呼），新增 `tags` 欄位儲存橫向屬性

## 1.117.2 - 2026-05-17

### 文件
- `baoyu-cover-image`：禁止用程式碼修補已生成的點陣圖文字 —— 不再使用 ImageMagick / Pillow / Canvas / SVG / HTML 疊層覆蓋、重寫或替換標題/副標題文字，文字異常時應改 prompt 重新生成或換用少字/無標題版本
- `baoyu-article-illustrator`、`baoyu-comic`、`baoyu-image-cards`、`baoyu-xhs-images`、`baoyu-infographic`、`baoyu-slide-deck`：同步上述文字修補禁令，各自針對該 skill 的文字類別（標籤/說明、對白/擬聲詞、標題/正文/標籤、標題/資料、幻燈片標題/要點）

## 1.117.1 - 2026-05-16

### 修復
- `baoyu-post-to-wechat`：修復微信瀏覽器文章釋出問題 (by @zhangga)
- `baoyu-post-to-wechat`：修復圖片上傳回退邏輯及 macOS WebP 剪貼簿複製

## 1.117.0 - 2026-05-16

### 新功能
- `baoyu-article-illustrator`：新增批次生成策略 —— 優先使用後端原生批次介面，其次執行時並行呼叫，最後順序生成；支援 `generation_batch_size` 配置和 `--batch-size` 引數
- `baoyu-comic`：新增批次生成策略，支援依賴感知排序（角色圖先於頁面）和 `--batch-size` 引數
- `baoyu-image-cards`：新增批次生成策略，遵循 image-1 錨定鏈，支援 `--batch-size` 引數
- `baoyu-slide-deck`：新增幻燈片圖片批次生成策略，支援 `--batch-size` 引數
- `baoyu-xhs-images`：同步 baoyu-image-cards 的批次生成策略

## 1.116.5 - 2026-05-14

### 新功能
- `baoyu-post-to-wechat`：當設定 `TELEGRAM_BOT_TOKEN` 和 `TELEGRAM_CHAT_ID` 環境變數時，自動將微信登入二維碼傳送到 Telegram，支援無顯示器/遠端登入場景 (by @beforesun)

### 重構
- `baoyu-post-to-wechat`：加固 Telegram QR 通知邏輯 —— 增加 10 秒 fetch 超時、未配置環境變數時不再無謂等待 2 秒、回退截圖改為視口範圍以減小體積

## 1.116.4 - 2026-05-14

### 重構
- `baoyu-wechat-summary`：精簡毒舌版提示詞（99 → 23 行），在 SKILL.md Round 2 中增加 roast 專用的畫像使用指引

## 1.116.3 - 2026-05-13

### 文件
- README 中將 Claude Code 替換為通用的 Agent 表述，體現多 Agent 支援（Claude Code、Codex 等）
...
### 新功能
- `baoyu-wechat-summary`：初始化設定流程中新增 `data_root` 選項，允許使用者在首次配置時自定義摘要輸出目錄

## 1.116.0 - 2026-05-13

### 新功能
- 新增 `baoyu-wechat-summary` 技能：將微信群聊精華提煉為結構化簡報，支援話題提取、發言排行榜和群友畫像。可生成正常版和毒舌版，支援增量模式和畫像回溯初始化。需安裝 [wx-cli](https://github.com/jackwener/wx-cli)。

## 1.115.4 - 2026-05-11

### 文件
- 圖片生成後端選擇規則強化：明確將 Codex `imagegen` 作為執行時原生工具的優先項（通過 `Skill` 工具呼叫，`skill: "imagegen"`），並禁止在無可用光柵後端時降級為 SVG/HTML/canvas 等程式碼渲染 —— 應退回到詢問使用者，而非靜默輸出程式碼繪圖。規則同步更新到 `docs/image-generation-tools.md`，並按自包含規則內聯到 `baoyu-article-illustrator`、`baoyu-comic`、`baoyu-cover-image`、`baoyu-image-cards`、`baoyu-infographic`、`baoyu-slide-deck`、`baoyu-xhs-images`。

## 1.115.3 - 2026-05-11

### 修復
- `baoyu-post-to-wechat`：修復微信編輯器中複製貼上前未啟用標籤頁的問題 (by @fengxiaodong28)
- `baoyu-post-to-x`：X 文章圖片插入改用工具欄媒體上傳替代剪貼簿貼上方式

## 1.115.2 - 2026-05-10

### 修復
- `baoyu-post-to-x`：將顯式請求 Codex Chrome 外掛的場景作為獨立瀏覽器控制模式處理，避免 Chrome Computer Use 或 CDP 回退流程靜默接管；同時改進 X Articles 草稿建立按鈕檢測。

## 1.115.1 - 2026-05-10

### 修復
- `baoyu-imagine`：將預設 MiniMax 圖片 API 端點改為 `https://api.minimaxi.com`，與當前官方圖片生成文件保持一致；仍可通過 `MINIMAX_BASE_URL` 覆蓋為 `https://api.minimax.io`。
- `baoyu-image-gen`：同步已廢棄圖片生成入口的 MiniMax 預設端點和迴歸測試。

## 1.115.0 - 2026-05-09

### 新功能
- `baoyu-post-to-x`：新增 Chrome Computer Use 作為 Codex 環境下的首選執行模式。當 Computer Use 工具可用時，所有 X 介面操作（發帖、文章、引用、影片）均通過使用者真實 Chrome 視窗完成，不再使用 CDP 指令碼。CDP 指令碼降級為 Computer Use 不可用或使用者明確要求時的回退方案。

## 1.114.1 - 2026-05-08

### 修復
- `baoyu-danger-gemini-web`：修復當前 Gemini Web 響應中生成圖 URL 以 `https://lh3.googleusercontent.com/gg-dl/` 形式出現、但不再包含舊版生成圖 marker 時的圖片提取失敗問題。補充該響應形態的迴歸測試。 (by @evilstar2016)

## 1.114.0 - 2026-05-05

### 新功能
- `baoyu-infographic`：新增 `retro-popup-pop` 風格 —— 復古畫素彈窗 × 波普資訊圖。畫面由多個 80/90 年代桌面彈窗疊加而成（標題欄、關閉按鈕、ERROR / ALERT 報錯對話方塊、`PROBLEMS.EXE` 等復古檔案窗、進度條、OK / CANCEL / FIX IT 按鈕），統一粗黑描邊、平塗色塊，背景使用亮青藍（#12B8DE）或復古奶油色（#F5F0E6）。與 `dense-modules` 佈局尤其契合；同時升級為 `高密度資訊大圖` 關鍵詞快捷方式與 `Product/Buying Guide` 內容型別的推薦風格。風格庫從 21 個擴充套件至 22 個。
  Credit to AJ@WaytoAGI.

### 文件
- `release-skills`：補充 GitHub Release 釋出流程，包括從 changelog 段落提取 release notes、建立 annotated tag、執行 `gh release create/edit`，以及為已有 tag 回填歷史 GitHub Releases。

## 1.113.0 - 2026-04-25

### 新功能
- `baoyu-imagine`：新增 DashScope Wan 2.7 影像模型支援（`wan2.7-image-pro` 與 `wan2.7-image`），通過阿里雲百鍊官方 API 直接呼叫，無需經 Replicate 轉發。支援文生圖、影像編輯、多圖融合（最多 9 張參考圖），按官方文件校驗 `[1:8, 8:1]` 寬高比範圍，並按模式應用不同的畫素預算規則。強制 `parameters.n: 1` 以匹配 baoyu-imagine 的單圖儲存語義，顯式拒絕 `--n > 1`，避免在使用者不知情的情況下產生多圖計費（API 在非拼圖模式下預設 `n=4`）。允許通過 `--provider dashscope --ref ...` 顯式啟用 Wan 2.7 參考圖工作流。
...
### 新功能
- `baoyu-url-to-markdown`：將 `baoyu-fetch` 執行時程式碼 vendored 到 `scripts/lib`，並通過本地 `scripts/baoyu-fetch` CLI 呼叫，使釋出後的技能安裝不再依賴 `baoyu-fetch` npm 包
...
### 維護
- 將 `defuddle` 升級到 0.17.0、`jsdom` 升級到 29.0.2，並通過 override 將 `@xmldom/xmldom` 固定到 0.8.13，清除 Defuddle 依賴鏈上的漏洞提示
...
### 新功能
- `baoyu-imagine`：新增 Z.AI GLM-Image 服務商支援，支援 `glm-image` 和 `cogview-4-250304` 模型，通過 Z.AI 同步影像 API 呼叫；配置 `ZAI_API_KEY`（或 `BIGMODEL_API_KEY` 向後相容）
...
### 修復
- `baoyu-slide-deck`：新增 `pptxgenjs` 依賴，PDF 合併時通過魔數字節檢測圖片格式替代副檔名判斷
...
### 修復
- `baoyu-post-to-wechat`：通過 magic bytes 檢測實際圖片格式，修復 CDN 返回與 URL 副檔名不一致的 content-type 問題（如 .png URL 實際返回 WebP）；WebP 格式按 PNG 策略處理以保留透明度
...
### 新功能
- `baoyu-fetch`：新增 X 會話 Cookie 旁路檔案，跨執行持久化登入狀態；通過 Browser.close 優雅關閉 Chrome；自動檢測並清理過期的 Chrome 配置鎖檔案
...
### 新功能
- `baoyu-youtube-transcript`：YouTube 封鎖直連 InnerTube API 時自動回退到 yt-dlp，支援備用客戶端身份重試及通過 `YOUTUBE_TRANSCRIPT_COOKIES_FROM_BROWSER` 環境變數傳遞瀏覽器 Cookie
...
### 新功能
- `baoyu-youtube-transcript`：新技能 — 下載 YouTube 影片字幕/轉錄文本和封面圖片，支援多語言、章節分段和說話人識別
...
### 新功能
- `baoyu-post-to-wechat`：新增多賬號支援，通過 `--account` 引數選擇賬號，EXTEND.md 支援 accounts 配置塊，每個賬號獨立 Chrome 配置目錄和憑證解析鏈
...
### 修復
- `baoyu-translate`：改進短文本註釋密度規則，補充風格預設到 02-prompt.md 的顯式傳遞
...
- `baoyu-comic`、`baoyu-xhs-images`：通過 skill 介面呼叫圖片生成，不再直接呼叫指令碼
...
### 修復
- `baoyu-post-to-wechat`：通過檔案選擇器攔截改進圖片上傳可靠性
...
### 新功能
- `baoyu-post-to-weibo`：新增微博釋出技能——支援帶圖文本釋出和頭條文章，通過 Chrome CDP 自動化操作
...
### 新功能
- `baoyu-post-to-wechat`：支援通過環境變數覆蓋瀏覽器除錯埠（`WECHAT_BROWSER_DEBUG_PORT`）和配置目錄（`WECHAT_BROWSER_PROFILE_DIR`）
- `baoyu-post-to-x`：支援通過環境變數覆蓋瀏覽器除錯埠（`X_BROWSER_DEBUG_PORT`）和配置目錄（`X_BROWSER_PROFILE_DIR`）
...
### 修復
- `baoyu-danger-x-to-markdown`：改進實體解析邏輯，通過邏輯鍵查詢提升媒體和連結映射準確性
...
### 重構
- `baoyu-article-illustrator`：最佳化 SKILL.md 從 197 行精簡至 150 行（減少 24%）；採用漸進式披露模式，主檔案提供簡潔概覽，詳細內容通過引用檔案提供。
...
### 重構
- `baoyu-article-illustrator`：簡化主 SKILL.md，將詳細步驟提取到 `workflow.md`；新增 Core Styles 快速選擇層（vector、minimal-flat、sci-fi、hand-drawn、editorial、scene）；新增 `vector-illustration` 作為推薦預設風格；新增插圖目的（information/visualization/imagination）以最佳化型別/風格推薦；在提示詞構建中新增預設構圖要求、人物渲染指南和文本樣式規則。
...
### 修復
- `baoyu-markdown-to-html`：生成帶 `data-local-path` 屬性的 `<img>` 標籤，而非純文本佔位符。
...
- `baoyu-post-to-wechat`：修復瀏覽器釋出模式，貼上前將 `<img>` 標籤轉換回文本佔位符。
...
### 新功能
- `baoyu-image-gen`：新增阿里雲通義萬象（DashScope）文生圖模型支援 (by @JianJang2017)。
...
### 新功能
- `baoyu-cover-image`：擴充套件寬高比選項——新增 4:3、3:2、3:4 比例；預設值從 2.35:1 改為 16:9 以提高通用性。現在除非通過 `--aspect` 標誌明確指定，否則始終確認寬高比。
...
### 文件
- `baoyu-comic`：改進 ohmsha 預設文件，明確預設哆啦 A 夢角色定義和視覺描述。
...
### 新功能
- `baoyu-cover-image`：新增使用者偏好設定支援（通過 EXTEND.md 配置）——可設定水印（內容、位置、透明度）、首選型別/風格、預設寬高比和自定義風格。新增 Step 0 檢查專案級（`.baoyu-skills/`）或使用者級（`~/.baoyu-skills/`）偏好設定，首次使用時引導設定。
...
### 新功能
- `baoyu-article-illustrator`：新增使用者偏好設定支援（通過 EXTEND.md 配置）——可設定水印（內容、位置、透明度）、首選型別/風格和自定義風格。新增 Step 1.1 檢查專案級（`.baoyu-skills/`）或使用者級（`~/.baoyu-skills/`）偏好設定，首次使用時引導設定。
...
### 新功能
- `baoyu-xhs-images`：新增使用者偏好設定支援（通過 EXTEND.md 配置）——可設定水印（內容、位置、透明度）、首選風格、首選佈局和自定義風格。新增 Step 0 檢查專案級（`.baoyu-skills/`）或使用者級（`~/.baoyu-skills/`）偏好設定，首次使用時引導設定。
...
### 新功能
- `baoyu-url-to-markdown`：新增 URL 轉 Markdown 工具技能，通過 Chrome CDP 抓取任意網頁並轉換為乾淨的 Markdown 格式。支援兩種抓取模式——自動模式（頁面載入後立即抓取）和等待模式（使用者控制抓取時機，適用於需要登入的頁面）。
...
### 修復
- `baoyu-danger-gemini-web`：改進 cookie 驗證邏輯，通過驗證實際 Gemini 會話可用性而非僅檢查 cookie 存在。
...
- 多原始檔支援：原始檔現以 `source-{slug}.{ext}` 命名，支援多個輸入（文本、圖片、會話中的檔案）。
...
### 新功能
- 擴充套件支援：所有技能現支援通過 `EXTEND.md` 檔案自定義。檢查 `.baoyu-skills/<skill-name>/EXTEND.md`（專案級）或 `~/.baoyu-skills/<skill-name>/EXTEND.md`（使用者級）配置自定義樣式與設定。
...
- `baoyu-comic`：新增 `scripts/merge-to-pdf.ts`，將封面/分頁圖片合併為 PDF；補充角色參考（圖片/文本）處理說明。
...
- `baoyu-post-to-x` / `baoyu-post-to-wechat`：新增 `scripts/paste-from-clipboard.ts`，通過系統級 Cmd/Ctrl+V 傳送“真實貼上”按鍵，規避 CDP 合成事件在站點側被忽略的問題。
...
- 初始釋出：提供 `.claude-plugin/marketplace.json` 與 `gemini-web`（文本/圖片生成、cookie 登入與快取流程）。
