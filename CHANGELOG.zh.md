...
### 新功能
- `baoyu-image-gen`：新增 `codex-cli` provider，封裝 `codex-imagegen` 後端，讓 Codex 內建的 `image_gen` 工具也能走標準的 `--provider` / 批次 / EXTEND.md 流程。使用使用者自己的 Codex 訂閱,無需 `OPENAI_API_KEY`。新增環境變數 `BAOYU_CODEX_IMAGEGEN_{BIN,CACHE_DIR,TIMEOUT_MS,RETRIES,LOG_FILE}`；`BAOYU_IMAGE_GEN_<PROVIDER>_*` 類覆蓋項支援帶連字元的 provider 名稱(如 `BAOYU_IMAGE_GEN_CODEX_CLI_CONCURRENCY`)。`codex-cli` **不會**自動選用,需透過 `--provider codex-cli` 或 EXTEND.md 的 `default_provider: codex-cli` 顯式指定
...
### 新功能
- `baoyu-markdown-to-html`：在標準圖片佔位符流水線之前,透過共享 Chrome（CDP）把 ` ```mermaid ` 圍欄程式碼塊渲染為本地 PNG。新增 CLI 引數 `--mermaid-theme <default|forest|dark|neutral|base>`、`--mermaid-scale <N>`（預設 `2`,即 @2x 解析度）、`--mermaid-bg <white|transparent|#hex>`,以及用於關閉渲染的 `--no-mermaid`。生成的圖片落在 `imgs/.mermaid-cache/mermaid-<hash>.png`,跨次執行透過對 `(code, theme, scale, background, mermaid 版本)` 取 SHA-256 前 12 位進行去重/複用。Chrome 不可用或單塊渲染失敗時,保留瀏覽器側 `<pre class="mermaid">` 兜底
...
- 透過斜槓命令呼叫時，請將 `/baoyu-imagine ...` 改為 `/baoyu-image-gen ...`；將所有 `baoyu-image-cards` 改為 `baoyu-xhs-images`
...
### 新功能
- `codex-imagegen`：新增面向非 Codex 執行時（如 Claude Code）的影像生成後端 —— 透過 `codex exec --json --sandbox danger-full-access` 呼叫 Codex CLI 內建的 `image_gen` 工具，無需 `OPENAI_API_KEY`。內建冪等快取、檔案鎖併發控制、JSONL 事件流解析、PNG 魔術位元組校驗和指數退避重試 (by @yelban, #158)
...
### 新功能
- `baoyu-post-to-wechat`：新增透過 SSH SOCKS5 隧道進行遠端 API 釋出
...
### 修復
- `baoyu-imagine`：將預設 MiniMax 圖片 API 端點改為 `https://api.minimaxi.com`，與當前官方圖片生成文件保持一致；仍可透過 `MINIMAX_BASE_URL` 覆蓋為 `https://api.minimax.io`。
- `baoyu-image-gen`：同步已廢棄圖片生成入口的 MiniMax 預設端點和迴歸測試。

## 1.115.0 - 2026-05-09

### 新功能
- `baoyu-post-to-x`：新增 Chrome Computer Use 作為 Codex 環境下的首選執行模式。當 Computer Use 工具可用時，所有 X 介面操作（發帖、文章、引用、影片）均透過使用者真實 Chrome 視窗完成，不再使用 CDP 指令碼。CDP 指令碼降級為 Computer Use 不可用或使用者明確要求時的回退方案。
...
### 新功能
- `baoyu-imagine`：新增 DashScope Wan 2.7 影像模型支援（`wan2.7-image-pro` 與 `wan2.7-image`），透過阿里雲百鍊官方 API 直接呼叫，無需經 Replicate 轉發。支援文生圖、影像編輯、多圖融合（最多 9 張參考圖），按官方文件校驗 `[1:8, 8:1]` 寬高比範圍，並按模式應用不同的畫素預算規則。強制 `parameters.n: 1` 以匹配 baoyu-imagine 的單圖儲存語義，顯式拒絕 `--n > 1`，避免在使用者不知情的情況下產生多圖計費（API 在非拼圖模式下預設 `n=4`）。允許透過 `--provider dashscope --ref ...` 顯式啟用 Wan 2.7 參考圖工作流。
...
### 新功能
- `baoyu-post-to-weibo`：新增微博釋出技能——支援帶圖文本釋出和頭條文章，透過 Chrome CDP 自動化操作
